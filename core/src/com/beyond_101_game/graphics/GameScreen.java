...
public class GameScreen implements Screen {
	...
	public void showDebugInfo() {
		...
		font.draw(sb,
				"Player Screen(x): " + (int) player.getScreencoordinates().x,
				5, 195);
		font.draw(sb,
				"Player Screen(y): " + (int) player.getScreencoordinates().y,
				5, 185);
		font.draw(sb, "Player World(x): "
				+ (int) player.getWorldcoordinates().x, 5, 170);
		font.draw(sb, "Player World(y): "
				+ (int) player.getWorldcoordinates().y, 5, 160);
		font.draw(sb, " Scroll Tracker x/y: " + (int) SCROLLTRACKER_X + " , "
				+ (int) SCROLLTRACKER_Y, 5, 140);
		TiledMapTileLayer.Cell cell = tilelayer.getCell(
				(int) player.getWorldcoordinates().x
						/ (int) tilelayer.getTileWidth(),
				(int) player.getWorldcoordinates().y
	...
	private void createElements() {
		...
		// origin
		cam.position.set(VIEWPORT_WIDTH / 2 + STARTOFFSET_X, VIEWPORT_HEIGHT
				/ 2 + STARTOFFSET_Y, 0);
